...
workingDir = './ICP_TestData/RandomTestData_PointCloud_PartialOverlap';
outputFolder = [];
% workingDir = '.';
% outputFolder = [];
...
% noiseInPlane   = {'0','0.5','1','0.5','1','0','0'};
% noisePerpPlane = {'0','0.5','1','1','0.5','0','0'};
% noiseEigValSqrt1 =   {'0','0.5'};
% noiseEigValSqrt23 =  {'0','0.5'};
% noiseType = { 'None','Surf' };
noiseEigValSqrt1 =   {'0.5'};
noiseEigValSqrt23 =  {'0.5'};
noiseType = { 'Surf' };
...
nNoises = size(noiseEigValSqrt1,2);
...
fprintf(fidSum,'%34s AngErr\t\t\tPosErr\t\t\t\tRuntime\t\t\t\tGTE\t\t\t\tNumIter\t\tNonConverged\tNumMaxIter\tNumMaxIterNonConverged\n\n',' ');
...
for offset = 1:nOffsets
  for noise = 1:nNoises
    ...
    testName = [... %['Noise_Prll',noiseInPlane{noise},'Perp',noisePerpPlane{noise},...
      'Noise_',noiseType{noise},'EigVal',noiseEigValSqrt1{noise},'-',noiseEigValSqrt23{noise},...
    ...
    for alg = 1:nAlgs
      ...
      fprintf(fidSum,'%-34s:\t\t%.2f/%.2f  %.2f/%.2f   %6.3f/%.3f   %.2f/%.2f   %4.1f/%-4.1f   %3u    %3u    %3u    (numTrials = %u)\n',...
...
save(dataFile,...  
  ...
  'noiseEigValSqrt1','noiseEigValSqrt23','noiseType',...
  'convThresh','maxIter')
